...
const Deposits = () => {
  ...
  const onSubmit = async (id, userid, amount, status) => {
    ...
    request.then(() => {
      ...
      sendNotification(
        userid,
        `Your deposit of Taka${amount} has been ${
  ...
  return (
    <div className="lg:p-6 py-6 space-y-6">
      ...
      {/* Filter Inputs */}
      <h2 className="text-xl font-semibold text-gray-200 py-2">
        Filter Deposits
      </h2>
            ...
            <tbody className="bg-gray-800 divide-y divide-gray-700">
              {currentItems?.map((deposit) => (
                <tr key={deposit.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-200">
                    {deposit.payment_phone_number}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-200">
                    {deposit.payment_method}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-200 bg-red-500">
                    {deposit?.ex2}{" "}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-200">
                    {deposit.amount} ৳
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <button
                      className={`btn ${
                        deposit.status == 0 ? "bg-green-500" : "bg-yellow-500"

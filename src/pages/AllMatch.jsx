...
const AllMatch = () => {
  ...
  useEffect(() => {
    ...
    if (filters.version !== "all") {
      filtered = filtered.filter(
        (match) => match.version.toLowerCase() === filters.version.toLowerCase()
      );
    }
  ...
  const ChangeStatuse = async (id, status) => {
    const request = axios.post(
      `${VITE_SERVER_API}/match-status/${id}/${status}`
    );
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {currentItems.length > 0 ? (
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="lg:px-6 pl-2 lg:py-4 whitespace-nowrap">
                        <img
                          ...
                          src={`${VITE_FILE_API}/${item.category?.image}`}
                          alt={item.name}
                        />
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-200">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-200">
                        <span
                          className={`${
                            SetStatus(item.date, item.time) == "Starting"
                              ? "bg-green-500"
                              : "bg-red-500"
                          } p-2 rounded-lg`}
                        >
                      ...
                      <td className="px-6 py-4 flex gap-2">
                        <button
                          disabled={
                            SetStatus(item.date, item.time) == "Ended"
                              ? false
                              : true
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            ChangeStatuse(
                              item.id,
                              parseInt(item.status == 1 ? 0 : 1)
                            )
                          }
                          className={`btn ${
                            item.status == 1 ? "bg-red-500" : "bg-green-500"
                          }`}
                        >
                  ...
                  <tr>
                    <td
                      colSpan="11"
                      className="px-6 py-4 text-sm text-gray-200 text-center"
                    >
                      {matchList.length === 0
                        ? "No matches found"

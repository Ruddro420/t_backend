...
const WithDrawRequests = () => {
  ...
  const fetchDeposits = () => {
    axios
      ...
      .then((res) => {
        const sorted = res.data.withdraw_requests.sort(
  ...
  const onSubmit = async (id, userid, amount, status) => {
    const request = axios.post(
      `${VITE_SERVER_API}/withdraw-request/${id}/${status}`
    );
    ...
    request.then(() => {
      ...
      sendNotification(
        userid,
        `Your withdraw request of Taka${amount} has been ${
                  ...
                  <td className="px-6 py-4 text-sm text-gray-200">
                    <span
                      className={`px-2 py-1 rounded-full text-xs ${
                        withdraw.status == 1 ? "bg-green-500" : withdraw.status==0? "bg-yellow-500" : "bg-red-500"
                      }`}
                    >
                      {withdraw.status == 1 ? "Approved" : withdraw.status == 0 ? "Pending" : "Cancelled"}
                    </span>
                  </td>
                  <td className="px-6 py-4 flex gap-2">
                    <button
                      className={`btn ${
                        withdraw.status == 0 ? "bg-green-500" : withdraw.status == 'cancelled' ? "bg-green-500" : "bg-yellow-500"
                      ...
                      onClick={() =>
                        onSubmit(
                          ...
                          withdraw.user_id,
                          withdraw.amount,
                          withdraw.status == 0 ? 1 : withdraw.status == 'cancelled' ? 1 : 0
                        )
                      }
                    >
                      {withdraw.status == 0 ? "Approve" : withdraw.status == 'cancelled'? "Approve" : "Set Pending"}
                    </button>
                    {withdraw.status != "cancelled" && (
                      <button
                        className={`btn bg-red-600 text-white px-3 py-1 rounded-md`}
                        onClick={() =>
                          onSubmit(
                            withdraw.id,
                            withdraw.user_id,
